...
function draw() {
  ...
  canvas.width = square_size * playing_board_columns;
  canvas.height = square_size * playing_board_rows;
  ...
  ctx.fillStyle = "rgb(0 0 200 / 50%)"; 
  if (x <= playing_board_columns && x >= 0) {
    draw_Shape();  // Draws the current falling shape
  }

  // Draw the landed shapes
  draw_landed_shapes();
...
function tracking_placed_shapes(x, y, collision) {
  // Place shape in landed array only if there was a collision
  if (collision) {
    for (let i = 0; i < shapes[shape_key].length; i++) {
      for (let j = 0; j < shapes[shape_key][i].length; j++) {
        if (shapes[shape_key][i][j] !== 0) {
          landed[y + i][x + j] = 1; // Place shape in landed array
        }
      }
    }
  }
